#!/usr/bin/env python3
"""
Ciena WaveServer Chassis/System Metric Collection Script for LogicMonitor

This script collects chassis-level metrics (software state) from Ciena WaveServer
optical transport devices via NETCONF. It outputs in LogicMonitor's single-instance
SCRIPT format (not BATCHSCRIPT).

Usage:
    ciena_chassis_collect.py <hostname> <username> <password> [--port PORT] [--debug]

Output Format (Single-instance - no instance prefix):
    datapoint_name=numeric_value

Example Output:
    software_oper_state=1
...
def load_config(config_path: str = None) -> dict:
    """
    Load the Ciena chassis configuration YAML file.
    ...
    if config_path is None:
        config_path = project_root / "configs" / "ciena_chassis.yaml"
...
def collect_chassis_metrics(
    ...
    """
    Collect chassis metrics from a Ciena WaveServer device.
    ...
    try:
        ...
        with NetconfClient(
            ...
            for metric_config in metric_configs:
                ...
                string_map = metric_config.get('string_map', {})
                ...
                if raw_value is not None:
                    ...
                    if string_map and raw_value in string_map:
                        value = float(string_map[raw_value])
                    elif string_map:
                        # Value not in string map, use 0
                        value = 0.0
                    else:
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Collect chassis metrics from Ciena WaveServer devices via NETCONF',
