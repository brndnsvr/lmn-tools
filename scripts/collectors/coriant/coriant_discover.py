#!/usr/bin/env python3
"""
Coriant Device Active Discovery Script for LogicMonitor

This script discovers optical interfaces (OTS, OMS, OSC, GOPT) on Coriant
optical transport devices via NETCONF and outputs them in LogicMonitor's
Active Discovery format.

Usage:
    coriant_discover.py <hostname> <username> <password> [--port PORT] [--debug]
...
def discover_interfaces(
    ...
    """
    Discover interfaces on a Coriant device and output in LogicMonitor format.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Discover optical interfaces on Coriant devices via NETCONF',
