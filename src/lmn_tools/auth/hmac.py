...
class LMv1Authenticator:
    """
    Reusable authenticator for LogicMonitor API requests.

    Provides a convenient interface for generating auth headers
    for multiple requests.

    Example:
        >>> auth = LMv1Authenticator("access_id", "access_key")
        >>> headers = auth.headers("GET", "/device/devices")
        >>> requests.get(url, headers=headers)
    """

    def __init__(self, access_id: str, access_key: str | SecretStr):
        """
    ...
    def auth_headers(
        ...
        """
        Generate AuthHeaders object for a request.
